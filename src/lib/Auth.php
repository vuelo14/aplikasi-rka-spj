...
class Auth {
  ...
  public static function requireLogin() { if (!self::check()) { header('Location: ' . APP_BASE_URL . 'login.php'); exit; } }
  public static function requireRole($roles=[]) { self::requireLogin(); if ($roles && !in_array($_SESSION['user']['role'], $roles)) { http_response_code(403); echo '<h3>Akses ditolak</h3>'; exit; } }
